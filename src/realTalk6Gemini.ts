/**
 * Real Talk 6 – OpenAI API 연동 (Favorite Movies)
 * /api/realtalk6-evaluate, /api/realtalk6-session-evaluate 호출.
...
console.log('[OpenAI API] Real Talk 6 AI 모듈 로드됨');
fetch('/api/realtalk6-available')
  .then((r) => r.json())
  .then((d) => console.log('[OpenAI API] Real Talk 6 연결 상태:', d?.available ? '연결됨 ✓' : '연결 안 됨 (mock 사용)'))
  .catch((e) => console.warn('[OpenAI API] Real Talk 6 연결 확인 실패:', e));
...
async function checkApiAvailable(): Promise<boolean> {
  ...
  try {
    const res = await fetch('/api/realtalk6-available');
...
export async function evaluateUserUtterance(
  ...
  try {
    const res = await fetchWithRetry('/api/realtalk6-evaluate', {
      method: 'POST',
...
export async function evaluateSession(
  ...
  try {
    const res = await fetchWithRetry('/api/realtalk6-session-evaluate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function evaluateCorrectionPractice(
  ...
  try {
    const res = await fetchWithRetry('/api/realtalk6-correction-practice', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
