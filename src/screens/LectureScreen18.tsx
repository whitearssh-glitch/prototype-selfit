/**
 * Screen 18 – Type 2-4 (Speed Up section)
 * Inserted after LectureScreen13 (index 14). Center text: "나는 피터예요." only (English hidden).
...
const WRONG_AUDIO = '/i-am-peter.mp3';
...
export function LectureScreen18({ onNext, hideSpeedDisplay, forceCorrect }: { onNext: () => void; hideSpeedDisplay?: boolean; forceCorrect?: boolean }) {
  ...
  const onResult = useCallback((transcript: string) => {
    ...
    if (said === expected || (said.includes('i am') && said.includes('peter')) || (said.includes('i m') && said.includes('peter'))) {
      setShowCheckmark(true);
      playDingDong();
      setTimeout(() => setShowCheckmark(false), 1200);
    } else {
