...
/**
 * Real Talk 11 – Ordering Hamburgers (Real Talk 7 기반) + 유저 턴별 힌트
 * - 턴 1,4,5: RT9 스타일 한글 프레임 (// 줄바꿈)
 * - 턴 2,3: RT8/10 스타일 영어 단어별 프레임 + 고정어 셔플
 */

import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { useSTT } from '../useSTT';
import { speak, stopSpeaking } from '../realTalk7TTS';
import { evaluateUserUtterance, getCathyFirstPhrase, type AIEvaluationResult } from '../realTalk7Gemini';
import type { ErrorLogItem, RealTalk7Data, SummaryItem } from '../realTalk7Types';

const REALTALK11_TOPIC = 'TOPIC: Ordering Hamburgers';
const REALTALK11_IMAGE = '/man3.png';
const STAR_POPUP_DELAY_MS = 2000;

const RT11_KO_HINTS: Record<number, string> = {
  0: '먹고 싶은 햄버거를 // 주문해 보세요.',
  3: '여기에서 먹을지 포장할지 // 점원에게 얘기해 보세요.',
  4: '고맙다고 인사하고 // 가격이 얼마인지 물어볼까요?',
};

const SIZE_HINTS = ['small', 'medium', 'large'] as const;
const SIDE_HINTS = ['French fries', 'soda', 'juice'] as const;
...
type HintVariant = 'none' | 'ko' | 'en';

type RealTalk11ScreenProps = {
  firstPhraseDone?: boolean;
  firstPhraseInProgress?: boolean;
  onComplete: (data: RealTalk7Data) => void;
};

type Phase = 'cathy' | 'user' | 'correction' | 'no-speech';

export function RealTalk11Screen({ firstPhraseDone = false, firstPhraseInProgress = false, onComplete }: RealTalk11ScreenProps) {
  ...
  const [hintVariant, setHintVariant] = useState<HintVariant>('none');
  const [koHintLines, setKoHintLines] = useState<string[] | null>(null);
  const [enHintLines, setEnHintLines] = useState<string[]>([]);
  ...
  const prepareHintsForUserTurn = useCallback((turnIdx: number) => {
    if (turnIdx === 1) {
      setHintVariant('en');
      setKoHintLines(null);
      setEnHintLines(shuffle3(SIZE_HINTS) as unknown as string[]);
      return;
    }
    if (turnIdx === 2) {
      setHintVariant('en');
      setKoHintLines(null);
      setEnHintLines(shuffle3([...SIDE_HINTS]));
      return;
    }
    const koRaw = RT11_KO_HINTS[turnIdx];
    if (koRaw) {
      setHintVariant('ko');
      setKoHintLines(koRaw.split('//').map((s) => s.trim()).filter(Boolean));
      setEnHintLines([]);
      return;
    }
    setHintVariant('none');
  ...
  const playCathyAndShowMic = useCallback(
    (phrase: string, phraseKo: string, showText: boolean, nextUserTurnIndex: number) => {
      ...
      prepareHintsForUserTurn(nextUserTurnIndex);
      speak(phrase, () => {
        setShowMic(true);
        setShowHints(true);
      });
    },
    [prepareHintsForUserTurn]
  );
    ...
    async (userText: string) => {
      ...
      if (result.correction) {
        ...
        const directionPhrase =
          result.correction.type === 'naturalness'
            ? 'So close! You can also say!'
            : result.correction.type === 'context'
  ...
  const showHintBelow = useMemo(() => {
    if (!showHints || !showMic || phase !== 'user' || showStarPopup) return false;
    if (hintVariant === 'ko' && koHintLines && koHintLines.length > 0) return true;
    if (hintVariant === 'en' && enHintLines.length >= 3) return true;
    return false;
  }, [showHints, showMic, phase, showStarPopup, hintVariant, koHintLines, enHintLines]);
    ...
    >
      <div className="realtalk2-layout realtalk-layout--reserve-go-space realtalk-layout--with-text-slots">
        <div className="realtalk-top">
          <div className="topic-box topic-box--step3">{REALTALK11_TOPIC}</div>
        </div>
        <div
          className={
        ...
        <div className="realtalk-main">
          <ThinkingImage src={REALTALK11_IMAGE} className="realtalk-main-image" evaluating={isEvaluating} />
        </div>

        <div className="realtalk2-text-below realtalk2-slot-two-lines" aria-hidden={!showHintBelow}>
          {showHintBelow && hintVariant === 'ko' && koHintLines ? (
            <div className="rt9-hint-frame" role="note" aria-label="Hint">
              <div>
                <span className="rt9-hint-text-sub">{koHintLines[0]}</span>
                {koHintLines[1] && (
                  <>
                    <br />
                    <span className="rt9-hint-text-sub">{koHintLines[1]}</span>
                  </>
                )}
              </div>
            </div>
          ) : showHintBelow && hintVariant === 'en' ? (
            <div className="realtalk-hints-frames-row" role="note" aria-label="Hints">
              <div className="realtalk-hint-word-frame realtalk-hint-word-frame--glow">
                <span className="realtalk-hint-word-frame-text">{enHintLines[0]}</span>
              </div>
              <div className="realtalk-hint-word-frame realtalk-hint-word-frame--glow">
                <span className="realtalk-hint-word-frame-text">{enHintLines[1]}</span>
              </div>
              <div className="realtalk-hint-word-frame realtalk-hint-word-frame--glow">
                <span className="realtalk-hint-word-frame-text">{enHintLines[2]}</span>
              </div>
            </div>
          ) : (
            <span className="realtalk2-text-placeholder" />
