/**
 * Screen 10 – Type 2-2 (Speed Up section)
 * Same layout as 2-1. Korean only visible at first; English hidden until mic recognition done, then appears with gradient.
 * Index 20: transcript check – correct → checkmark + next; wrong → wrong mark + nice-to-meet-you.mp3, then next.
 */
...
const SCREEN10_WRONG_AUDIO = '/nice-to-meet-you.mp3';
...
export function LectureScreen10({ onNext, hideSpeedDisplay }: { onNext: () => void; hideSpeedDisplay?: boolean }) {
  ...
  const onResult = useCallback((transcript: string) => {
    ...
    if (said === expected) {
      ...
      /* 느낌표 잠깐 표시 후 완전히 없애고, 텍스트 그라데이션 + 음원 동시에 */
      setTimeout(() => {
  ...
  /* 오답 후 정답 문장 그라데이션 표시 → 일정 시간 뒤 다음 화면 */
  useEffect(() => {
