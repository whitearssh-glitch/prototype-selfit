/**
 * Real Talk 5 – 세션 평가 (Meet New Friends)
 * 세션 평가: 주제 적합도, 표현 적절성 (별점), 총평 (한국어).
 */

import { useEffect } from 'react';
import { evaluateSession } from '../realTalk5Gemini';
import type { ErrorLogItem, SessionEvaluation, SummaryItem } from '../realTalk5Types';
...
type RealTalk5EvaluationScreenProps = {
...
export function RealTalk5EvaluationScreen({
  evaluation,
  onEvaluationLoaded,
  conversationSummary,
  errorLog,
  onNext,
}: RealTalk5EvaluationScreenProps) {
  useEffect(() => {
    if (evaluation === null) {
  ...
  if (evaluation === null) {
    return (
      <div className="screen-content screen-content--step3-colors-no-frame" data-screen="rt5-eval">
