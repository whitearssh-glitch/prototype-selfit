...
/**
 * Real Talk 9 – Meet New Friends (Real Talk 5 기반) + 한글 고정 힌트 프레임
 *
 * - 힌트 노출: AI TTS 종료 후, 유저 턴 시작(마이크 ON) 시점
 * - 힌트 스타일: RT9 전용 한글 프레임(꼬리 없음, 문장 길이 맞춤, // 는 줄바꿈)
...
import { evaluateUserUtterance, getCathyFirstPhrase, type AIEvaluationResult } from '../realTalk5Gemini';
import type { ErrorLogItem, RealTalk5Data, SummaryItem } from '../realTalk5Types';

const REALTALK9_TOPIC = 'TOPIC: Meet New Friends';
const REALTALK9_IMAGE = '/girl1.png';
const STAR_POPUP_DELAY_MS = 2000;

const RT9_FIXED_HINTS_KO: Record<number, string> = {
  0: '만나서 반갑다고 인사해 볼까요?',
  1: '이름을 소개해 보세요. // My name is ~',
  2: "나이를 말해 보세요. // I'm ~ years old.",
};
...
type RealTalk9ScreenProps = {
  firstPhraseDone?: boolean;
  firstPhraseInProgress?: boolean;
  onComplete: (data: RealTalk5Data) => void;
};

type Phase = 'cathy' | 'user' | 'correction' | 'no-speech';

export function RealTalk9Screen({ firstPhraseDone = false, firstPhraseInProgress = false, onComplete }: RealTalk9ScreenProps) {
  ...
  const handleUserUtterance = useCallback(
    async (userText: string) => {
      ...
      if (!t) {
        ...
        setPhase('no-speech');
        stopSpeaking();
        speakRealTalk5(cathyPhraseRef.current, () => {
          setShowMic(true);
          setShowHints(true);
        });
        return;
      ...
      if (result.correction) {
        ...
        speakRealTalk5(directionPhrase, () => {
          ...
          setShowTextAbove(true);
          speakRealTalk5(result.correction!.sentence, () => {
            setShowMic(true);
            setShowHints(false);
          });
        });
  ...
  const rt9HintKo = useMemo(() => RT9_FIXED_HINTS_KO[userTurnIndex] ?? null, [userTurnIndex]);
  const rt9HintLines = useMemo(() => {
    if (!rt9HintKo) return null;
    return rt9HintKo.split('//').map((s) => s.trim()).filter(Boolean);
  }, [rt9HintKo]);

  const showHintFrame = Boolean(rt9HintLines?.length) && showHints && phase === 'user' && showMic && !showStarPopup;
    ...
    >
      <div className="realtalk2-layout realtalk-layout--reserve-go-space realtalk-layout--with-text-slots">
        <div className="realtalk-top">
          <div className="topic-box topic-box--step3">{REALTALK9_TOPIC}</div>
        </div>
        ...
        <div className="realtalk-main">
          <ThinkingImage src={REALTALK9_IMAGE} className="realtalk-main-image" evaluating={isEvaluating} />
        </div>

        <div className="realtalk2-text-below realtalk2-slot-two-lines" aria-hidden={!showHintFrame}>
          {showHintFrame && rt9HintLines ? (
            <div className="rt9-hint-frame" role="note" aria-label="Hint">
              <div>
                <span>{rt9HintLines[0]}</span>
                {rt9HintLines[1] && (
                  <>
                    <br />
                    <span className="rt9-hint-text-sub">{rt9HintLines[1]}</span>
                  </>
                )}
              </div>
            </div>
