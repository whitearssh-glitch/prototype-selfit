/**
 * Real Talk 4 – 교정 연습 (Grocery Shopping)
 */

import { useCallback, useEffect, useRef, useState } from 'react';
import { useSTT } from '../useSTT';
import { speakRealTalk4, stopSpeaking } from '../realTalk3TTS';
import { evaluateCorrectionPractice } from '../realTalk4Gemini';
import { getCorrectionPracticeItems } from '../realTalk4Types';
import type { ErrorLogItem } from '../realTalk4Types';
...
type RealTalk4CorrectionPracticeScreenProps = {
  items: ErrorLogItem[];
  onComplete: () => void;
};

export function RealTalk4CorrectionPracticeScreen({ items, onComplete }: RealTalk4CorrectionPracticeScreenProps) {
    ...
    async (transcript: string) => {
      ...
      if (nextAttempt === 1) {
        ...
        setTimeout(() => {
          setIsBlinking(false);
          stopSpeaking();
          speakRealTalk4(current.corrected, () => setShowMic(true));
        }, 1000);
      } else {
        setTextKey((k) => k + 1);
        setShowText(true);
        stopSpeaking();
        speakRealTalk4(current.corrected, () => {
          setTimeout(() => {
            setShowStar(true);
            playDingDong();
            setTimeout(() => {
  ...
  useEffect(() => {
    ...
    setShowText(true);
    setShowMic(false);
    speakRealTalk4(items[0].corrected, () => setShowMic(true));
    return () => stopSpeaking();
  }, []);
  ...
  useEffect(() => {
    ...
    stopSpeaking();
    setShowMic(false);
    speakRealTalk4(item.corrected, () => setShowMic(true));
    return () => stopSpeaking();
  }, [index]);
        ...
        >
          <div className="screen7-stamp-circle recap-good-stamp-circle">
            <svg className="screen7-stamp-svg" viewBox="0 0 120 120" aria-hidden>
              <defs>
                <path id="recap-correction-stamp-star-rt4" d="M0 -2.1 L0.5 -0.5 L2.2 -0.5 L0.8 0.5 L1.3 2.1 L0 1.1 L-1.3 2.1 L-0.8 0.5 L-2.2 -0.5 L-0.5 -0.5 Z" fill="#fff" />
              </defs>
              <circle cx="60" cy="60" r="52" fill="none" stroke="#fff" strokeWidth={3} />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(98, 38) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(82, 22) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(60, 16) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(38, 22) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(22, 38) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(22, 82) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(38, 98) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(60, 104) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(82, 98) scale(2)" />
              <use href="#recap-correction-stamp-star-rt4" transform="translate(98, 82) scale(2)" />
