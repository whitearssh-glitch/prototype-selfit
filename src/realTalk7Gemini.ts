/**
 * Real Talk 7 – OpenAI API 연동 (Ordering Hamburgers)
 * /api/realtalk7-evaluate, /api/realtalk7-session-evaluate 호출.
...
console.log('[OpenAI API] Real Talk 7 AI 모듈 로드됨');
fetch('/api/realtalk7-available')
  .then((r) => r.json())
  .then((d) => console.log('[OpenAI API] Real Talk 7 연결 상태:', d?.available ? '연결됨 ✓' : '연결 안 됨 (mock 사용)'))
  .catch((e) => console.warn('[OpenAI API] Real Talk 7 연결 확인 실패:', e));
...
async function checkApiAvailable(): Promise<boolean> {
  ...
  try {
    const res = await fetch('/api/realtalk7-available');
...
export async function evaluateUserUtterance(
  ...
  try {
    const res = await fetchWithRetry('/api/realtalk7-evaluate', {
      method: 'POST',
  ...
  return {
    ...
    correction:
      data.correction && typeof data.correction === 'object'
        ? {
            type: (() => {
              const t = (data.correction as { type?: string }).type;
              return t === 'context' ? 'context' : t === 'naturalness' ? 'naturalness' : 'grammar';
            })(),
...
export async function evaluateSession(
  ...
  try {
    const res = await fetchWithRetry('/api/realtalk7-session-evaluate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function evaluateCorrectionPractice(
  ...
  try {
    const res = await fetchWithRetry('/api/realtalk7-correction-practice', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
